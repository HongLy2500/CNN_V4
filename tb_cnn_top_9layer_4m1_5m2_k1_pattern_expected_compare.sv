...
module tb_cnn_top_9layer_4m1_5m2_k1_pattern_expected_compare;
  ...
  //   - Use non-uniform first-layer input and different per-layer weights so the
  //     final OFM is not a constant 127 saturation pattern.
  //
  // Shape/data model:
  //   H=W are kept constant by K=1/no-pool in all layers.  C=F=1 keeps the
  //   golden model simple and avoids saturation, while W=72 (> PC=32) still
  //   forces Mode2 rolling horizontal refill.
  //
  //   input(row,col) = 1 + ((2*row + 3*col) % 15)
  //   layer weights  = {1,2,1,1,2,1,1,2,1}
  //   final(row,col) = input(row,col) * 8, exact no-saturation expected.
  // --------------------------------------------------------------------------
  ...
  localparam int H_MAX = 24;
  localparam int W_MAX = 72;
  localparam int HT = 4;
  ...
  // All layers use the same spatial/channel dimensions.
  localparam int LH = 24;
  localparam int LW = 72;
  localparam int LC = 1;
  localparam int LF = 1;
  localparam int LK = 1;
  localparam int LPOOL = 0;

  localparam int L0_H_IN=LH, L0_W_IN=LW, L0_C_IN=LC, L0_F_OUT=LF, L0_K=LK, L0_POOL_EN=LPOOL;
  localparam int L0_H_CONV_OUT=LH, L0_W_CONV_OUT=LW, L0_H_OUT=LH, L0_W_OUT=LW;
  localparam int L1_H_IN=LH, L1_W_IN=LW, L1_C_IN=LC, L1_F_OUT=LF, L1_K=LK, L1_POOL_EN=LPOOL;
  localparam int L1_H_CONV_OUT=LH, L1_W_CONV_OUT=LW, L1_H_OUT=LH, L1_W_OUT=LW;
  localparam int L2_H_IN=LH, L2_W_IN=LW, L2_C_IN=LC, L2_F_OUT=LF, L2_K=LK, L2_POOL_EN=LPOOL;
  localparam int L2_H_CONV_OUT=LH, L2_W_CONV_OUT=LW, L2_H_OUT=LH, L2_W_OUT=LW;
  localparam int L3_H_IN=LH, L3_W_IN=LW, L3_C_IN=LC, L3_F_OUT=LF, L3_K=LK, L3_POOL_EN=LPOOL;
  localparam int L3_H_CONV_OUT=LH, L3_W_CONV_OUT=LW, L3_H_OUT=LH, L3_W_OUT=LW;
  localparam int L4_H_IN=LH, L4_W_IN=LW, L4_C_IN=LC, L4_F_OUT=LF, L4_K=LK, L4_POOL_EN=LPOOL;
  localparam int L4_H_CONV_OUT=LH, L4_W_CONV_OUT=LW, L4_H_OUT=LH, L4_W_OUT=LW;
  localparam int L5_H_IN=LH, L5_W_IN=LW, L5_C_IN=LC, L5_F_OUT=LF, L5_K=LK, L5_POOL_EN=LPOOL;
  localparam int L5_H_CONV_OUT=LH, L5_W_CONV_OUT=LW, L5_H_OUT=LH, L5_W_OUT=LW;
  localparam int L6_H_IN=LH, L6_W_IN=LW, L6_C_IN=LC, L6_F_OUT=LF, L6_K=LK, L6_POOL_EN=LPOOL;
  localparam int L6_H_CONV_OUT=LH, L6_W_CONV_OUT=LW, L6_H_OUT=LH, L6_W_OUT=LW;
  localparam int L7_H_IN=LH, L7_W_IN=LW, L7_C_IN=LC, L7_F_OUT=LF, L7_K=LK, L7_POOL_EN=LPOOL;
  localparam int L7_H_CONV_OUT=LH, L7_W_CONV_OUT=LW, L7_H_OUT=LH, L7_W_OUT=LW;
  localparam int L8_H_IN=LH, L8_W_IN=LW, L8_C_IN=LC, L8_F_OUT=LF, L8_K=LK, L8_POOL_EN=LPOOL;
  localparam int L8_H_CONV_OUT=LH, L8_W_CONV_OUT=LW, L8_H_OUT=LH, L8_W_OUT=LW;

  localparam int L0_NUM_CGROUP=(LC+PV_M1-1)/PV_M1, L0_NUM_FGROUP=(LF+PF_M1-1)/PF_M1;
  localparam int L1_NUM_CGROUP=(LC+PV_M1-1)/PV_M1, L1_NUM_FGROUP=(LF+PF_M1-1)/PF_M1;
  localparam int L2_NUM_CGROUP=(LC+PV_M1-1)/PV_M1, L2_NUM_FGROUP=(LF+PF_M1-1)/PF_M1;
  localparam int L3_NUM_CGROUP=(LC+PV_M1-1)/PV_M1, L3_NUM_FGROUP=(LF+PF_M1-1)/PF_M1;
  localparam int L4_NUM_CGROUP=(LC+PC-1)/PC,     L4_NUM_FGROUP=(LF+PF-1)/PF;
  localparam int L5_NUM_CGROUP=(LC+PC-1)/PC,     L5_NUM_FGROUP=(LF+PF-1)/PF;
  localparam int L6_NUM_CGROUP=(LC+PC-1)/PC,     L6_NUM_FGROUP=(LF+PF-1)/PF;
  localparam int L7_NUM_CGROUP=(LC+PC-1)/PC,     L7_NUM_FGROUP=(LF+PF-1)/PF;
  localparam int L8_NUM_CGROUP=(LC+PC-1)/PC,     L8_NUM_FGROUP=(LF+PF-1)/PF;

  localparam int L0_WGT_WORDS=L0_NUM_FGROUP*L0_NUM_CGROUP*LK*LK;
  localparam int L1_WGT_WORDS=L1_NUM_FGROUP*L1_NUM_CGROUP*LK*LK;
  localparam int L2_WGT_WORDS=L2_NUM_FGROUP*L2_NUM_CGROUP*LK*LK;
  localparam int L3_WGT_WORDS=L3_NUM_FGROUP*L3_NUM_CGROUP*LK*LK;
  ...
  localparam int L0_COLBLKS=(LW+PV_M1-1)/PV_M1;
  localparam int L8_STORED_GROUPS=(LW+PC-1)/PC;
  localparam int EXPECTED_IFM_DDR_READS = L0_COLBLKS * LH * LC;
  ...
  localparam int EXPECTED_OFM_DDR_WORDS = LF * LH * L8_STORED_GROUPS;
  localparam int EXPECTED_FINAL_ELEMENTS = LF * LH * LW;
  localparam int EXPECTED_WEIGHT_PRODUCT = 8;
  ...
  function automatic int layer_weight_coeff(input int layer_id);
    begin
      case (layer_id)
        0: layer_weight_coeff = 1;
        1: layer_weight_coeff = 2;
        2: layer_weight_coeff = 1;
        3: layer_weight_coeff = 1;
        4: layer_weight_coeff = 2;
        5: layer_weight_coeff = 1;
        6: layer_weight_coeff = 1;
        7: layer_weight_coeff = 2;
        default: layer_weight_coeff = 1;
      endcase
    end
  endfunction

  function automatic int input_pattern_val(input int row, input int col);
    begin
      input_pattern_val = 1 + ((2*row + 3*col) % 15);
    end
  endfunction
  ...
  function automatic logic [DATA_W-1:0] expected_final_byte(input int row, input int col);
    begin
      expected_final_byte = sat8(input_pattern_val(row, col) * EXPECTED_WEIGHT_PRODUCT);
    end
  endfunction

  function automatic logic [DDR_WORD_W-1:0] pack_m1_ifm_word(input int row, input int col_blk);
    logic [DDR_WORD_W-1:0] word;
    int col;
    begin
      word = '0;
      for (int lane = 0; lane < DDR_LANES; lane++) begin
        col = col_blk * PV_M1 + lane;
        if ((lane < PV_M1) && (row < LH) && (col < LW)) begin
          word[lane*DATA_W +: DATA_W] = sat8(input_pattern_val(row, col));
        end
      end
      return word;
    end
  ...
  function automatic logic [DDR_WORD_W-1:0] pack_weight_word(input int coeff);
    logic [DDR_WORD_W-1:0] word;
    begin
      word = '0;
      for (int lane = 0; lane < DDR_LANES; lane++) begin
        word[lane*DATA_W +: DATA_W] = sat8(coeff);
      end
      return word;
    end
  endfunction
  ...
  function automatic logic [DDR_WORD_W-1:0] expected_final_word(input int word_idx);
    ...
    begin
      word = '0;
      words_per_ch = LH * L8_STORED_GROUPS;
      ...
      for (int lane = 0; lane < DDR_LANES; lane++) begin
        col_g = grp * PC + lane;
        if ((ch < LF) && (row < LH) && (lane < PC) && (col_g < LW)) begin
          word[lane*DATA_W +: DATA_W] = expected_final_byte(row, col_g);
        end
      end
      return word;
    end
  ...
  task automatic init_mem;
    ...
    begin
      ...
      // C=1 in this regression, so the address formula is unambiguous.
      for (row = 0; row < LH; row = row + 1) begin
        for (col_blk = 0; col_blk < L0_COLBLKS; col_blk = col_blk + 1) begin
          addr = `DDR_IFM_BASE + row * L0_COLBLKS + col_blk;
          ddr_mem[addr] = pack_m1_ifm_word(row, col_blk);
        end
      end

      for (i = 0; i < L0_WGT_DDR_WORDS; i = i + 1) ddr_mem[L0_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(0));
      for (i = 0; i < L1_WGT_DDR_WORDS; i = i + 1) ddr_mem[L1_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(1));
      for (i = 0; i < L2_WGT_DDR_WORDS; i = i + 1) ddr_mem[L2_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(2));
      for (i = 0; i < L3_WGT_DDR_WORDS; i = i + 1) ddr_mem[L3_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(3));
      for (i = 0; i < L4_WGT_DDR_WORDS; i = i + 1) ddr_mem[L4_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(4));
      for (i = 0; i < L5_WGT_DDR_WORDS; i = i + 1) ddr_mem[L5_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(5));
      for (i = 0; i < L6_WGT_DDR_WORDS; i = i + 1) ddr_mem[L6_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(6));
      for (i = 0; i < L7_WGT_DDR_WORDS; i = i + 1) ddr_mem[L7_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(7));
      for (i = 0; i < L8_WGT_DDR_WORDS; i = i + 1) ddr_mem[L8_WGT_DDR_BASE + i] = pack_weight_word(layer_weight_coeff(8));
    end
  endtask
  ...
  task automatic program_layers;
    ...
    begin
      fill_layer_cfg(cfg0, 0, 1'b0, LH,LW,LC,LF,LK,LH,LW,LPOOL,L0_WGT_DDR_BASE, 1'b1, 1'b0);
      fill_layer_cfg(cfg1, 1, 1'b0, LH,LW,LC,LF,LK,LH,LW,LPOOL,L1_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg2, 2, 1'b0, LH,LW,LC,LF,LK,LH,LW,LPOOL,L2_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg3, 3, 1'b0, LH,LW,LC,LF,LK,LH,LW,LPOOL,L3_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg4, 4, 1'b1, LH,LW,LC,LF,LK,LH,LW,LPOOL,L4_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg5, 5, 1'b1, LH,LW,LC,LF,LK,LH,LW,LPOOL,L5_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg6, 6, 1'b1, LH,LW,LC,LF,LK,LH,LW,LPOOL,L6_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg7, 7, 1'b1, LH,LW,LC,LF,LK,LH,LW,LPOOL,L7_WGT_DDR_BASE, 1'b0, 1'b0);
      fill_layer_cfg(cfg8, 8, 1'b1, LH,LW,LC,LF,LK,LH,LW,LPOOL,L8_WGT_DDR_BASE, 1'b0, 1'b1);
      write_cfg(0, cfg0);
      write_cfg(1, cfg1);
      write_cfg(2, cfg2);
      write_cfg(3, cfg3);
  ...
  task automatic check_final_ofm;
    ...
    begin
      ...
      if (mismatch != 0) begin
        dump_ofm_region();
        $fatal(1, "TB_FAIL: 4M1+5M2 patterned final OFM mismatch count=%0d", mismatch);
      end
    end
  endtask
  ...
  task automatic print_banner;
    begin
      $display("TB_INFO: 9-layer 4xMode1 + 5xMode2 K=1 patterned expected-compare test");
      $display("TB_INFO: L0-L3 MODE1, L4-L8 MODE2, HxW=%0dx%0d C=F=%0d", LH, LW, LC);
      $display("TB_INFO: input(row,col)=1+((2*row+3*col)%%15), weights={1,2,1,1,2,1,1,2,1}, product=%0d", EXPECTED_WEIGHT_PRODUCT);
      $display("TB_INFO: expected final OFM words=%0d logical elements=%0d", EXPECTED_OFM_DDR_WORDS, EXPECTED_FINAL_ELEMENTS);
      $display("TB_INFO: W=%0d > PC=%0d, so Mode2 layers require rolling horizontal refill", LW, PC);
    end
  endtask
  ...
  initial begin
    ...
    check_final_ofm();
    $display("TB_PASS: 9-layer 4M1+5M2 K1 patterned expected-compare passed. final_product=%0d final_words=%0d", EXPECTED_WEIGHT_PRODUCT, EXPECTED_OFM_DDR_WORDS);
    $finish;
  end
