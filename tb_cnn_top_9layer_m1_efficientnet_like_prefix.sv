...
module tb_cnn_top_9layer_m1_efficientnet_like_prefix;
  ...
  localparam int C_MAX=16, F_MAX=16, W_MAX=96, H_MAX=96, HT=4, K_MAX=5;
  ...
  // The goal is to stress the first 9 mode-1 layers: RGB input, K=3/K=5/K=1, many OFM->IFM handoffs, partial C/F groups, and H/W tiling.
  // L0: 96x96x3 -> conv 94x94x8 K=3 -> pool 47x47x8
  // L1: 47x47x8 -> conv 45x45x8 K=3 -> no pool
  // L2: 45x45x8 -> conv 41x41x10 K=5 -> pool 20x20x10
  // L3: 20x20x10 -> conv 18x18x12 K=3 -> no pool
  // L4: 18x18x12 -> conv 14x14x13 K=5 -> pool 7x7x13
  // L5: 7x7x13 -> conv 5x5x15 K=3 -> no pool
  // L6: 5x5x15 -> conv 3x3x16 K=3 -> no pool
  // L7: 3x3x16 -> conv 3x3x16 K=1 -> no pool
  // L8: 3x3x16 -> conv 3x3x11 K=1 -> pool 1x1x11
  ...
  localparam int L2_H_IN=45, L2_W_IN=45, L2_C_IN=8, L2_F_OUT=10, L2_K=5, L2_POOL_EN=1;
  ...
  localparam int L3_H_IN=20, L3_W_IN=20, L3_C_IN=10, L3_F_OUT=12, L3_K=3, L3_POOL_EN=0;
  ...
  localparam int L4_H_IN=18, L4_W_IN=18, L4_C_IN=12, L4_F_OUT=13, L4_K=5, L4_POOL_EN=1;
  ...
  localparam int L5_H_IN=7, L5_W_IN=7, L5_C_IN=13, L5_F_OUT=15, L5_K=3, L5_POOL_EN=0;
  ...
  localparam int L6_H_IN=5, L6_W_IN=5, L6_C_IN=15, L6_F_OUT=16, L6_K=3, L6_POOL_EN=0;
  ...
  localparam int L7_H_IN=3, L7_W_IN=3, L7_C_IN=16, L7_F_OUT=16, L7_K=1, L7_POOL_EN=0;
  ...
  localparam int L8_H_IN=3, L8_W_IN=3, L8_C_IN=16, L8_F_OUT=11, L8_K=1, L8_POOL_EN=1;
...
// ============================================================
// DEBUG: Layer 2 stuck diagnosis for EfficientNet-like test
// Paste into tb_cnn_top_9layer_m1_efficientnet_like_prefix.sv
// ============================================================

integer dbg_l2_cycle_count;
integer dbg_l2_stream_start_count;
integer dbg_l2_stream_done_count;
integer dbg_l2_ofm_write_pulse_count;
integer dbg_l2_ofm_valid_elem_count;
integer dbg_l2_free_count;
logic   dbg_l2_m1_done_seen;
logic   dbg_l2_ofm_layer_done_seen;

always_ff @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    dbg_l2_cycle_count           <= 0;
    dbg_l2_stream_start_count    <= 0;
    dbg_l2_stream_done_count     <= 0;
    dbg_l2_ofm_write_pulse_count <= 0;
    dbg_l2_ofm_valid_elem_count  <= 0;
    dbg_l2_free_count            <= 0;
    dbg_l2_m1_done_seen          <= 1'b0;
    dbg_l2_ofm_layer_done_seen   <= 1'b0;
  end else begin
    if (dut.dbg_layer_idx == 2) begin
      dbg_l2_cycle_count <= dbg_l2_cycle_count + 1;

      if (dut.ofm_ifm_stream_start_s) begin
        dbg_l2_stream_start_count <= dbg_l2_stream_start_count + 1;

        if (dbg_l2_stream_start_count < 40 ||
            (dbg_l2_stream_start_count % 100) == 0) begin
          $display("DBG_L2_STREAM_START t=%0t cnt=%0d row=%0d col_base=%0d slot=%0d ch_blk=%0d",
                   $time,
                   dbg_l2_stream_start_count + 1,
      ...
      if (dut.ofm_ifm_stream_done_s) begin
        dbg_l2_stream_done_count <= dbg_l2_stream_done_count + 1;

        if (dbg_l2_stream_done_count < 40 ||
            (dbg_l2_stream_done_count % 100) == 0) begin
          $display("DBG_L2_STREAM_DONE t=%0t cnt=%0d",
                   $time,
                   dbg_l2_stream_done_count + 1);
        end
      end
      ...
      if (dut.m1_done_s && !dbg_l2_m1_done_seen) begin
        dbg_l2_m1_done_seen <= 1'b1;
        $display("DBG_L2_M1_DONE t=%0t", $time);
      end

      if (dut.ofm_layer_write_done_s && !dbg_l2_ofm_layer_done_seen) begin
        dbg_l2_ofm_layer_done_seen <= 1'b1;
        $display("DBG_L2_OFM_LAYER_WRITE_DONE t=%0t", $time);
      end

if ((dbg_l2_cycle_count % 100000) == 0) begin
  $display("DBG_L2_STATUS t=%0t cycle=%0d stream_start=%0d stream_done=%0d ofm_write_pulse=%0d ofm_valid_elem=%0d free=%0d m1_busy=%0b m1_done=%0b ofm_done=%0b m1_step=%0b out_row=%0d out_col=%0d f_group=%0d c_iter=%0d ky=%0d kx=%0d",
           $time,
           dbg_l2_cycle_count,
           dbg_l2_stream_start_count,
           dbg_l2_stream_done_count,
...
final begin
  $display("DBG_L2_SUMMARY stream_start=%0d expected_L1_to_L2_total=2070",
           dbg_l2_stream_start_count);
  $display("DBG_L2_SUMMARY stream_done=%0d expected_L1_to_L2_total=2070",
           dbg_l2_stream_done_count);
  $display("DBG_L2_SUMMARY ofm_write_pulse=%0d expected_L2_write_pulses=1200",
           dbg_l2_ofm_write_pulse_count);
  $display("DBG_L2_SUMMARY ofm_valid_elem=%0d expected_L2_valid_elements=4000",
           dbg_l2_ofm_valid_elem_count);
  $display("DBG_L2_SUMMARY free_count=%0d m1_done_seen=%0b ofm_layer_done_seen=%0b",
           dbg_l2_free_count,
           dbg_l2_m1_done_seen,
           dbg_l2_ofm_layer_done_seen);
end
  ...
  task automatic init_mem;
    begin
      ...
      $display("TB_INFO: benchmark intent: EfficientNet-like mode-1 prefix; K=3/K=5/K=1; no depthwise/SE/skip ops");
