...
// ============================================================================
// Initial-IFM tiled top-level testbench for cnn_top, MODE 1 only
//
// Purpose:
// - Exercise a first-layer mode-1 IFM that is taller than the IFM buffer
//   tile height HT, forcing the design to use initial DDR->IFM tiling/refill
//   within the same layer.
// - This is different from OFM->IFM same-mode refill between layers.
// - The checker compares final DDR OFM against a golden Conv+ReLU+MaxPool
//   model and also checks that more than one IFM DMA command was issued.
//
...
module tb_cnn_top_m1_initial_ifm_tiled;
  ...
  localparam int HT               = 4;  // intentionally smaller than L_H_IN
  ...
  localparam int MAX_CYCLES       = 300000;

  // --------------------------------------------------------------------------
  // Benchmark-like layer shape
  // --------------------------------------------------------------------------
  localparam int L_H_IN           = 8;  // H_in > HT => initial IFM tiling/refill required
  localparam int L_W_IN           = 6;
  localparam int L_C_IN           = 3;
  localparam int L_F_OUT          = 8;
  localparam int L_K              = 3;
  localparam int L_H_CONV_OUT     = L_H_IN - L_K + 1;   // 6
  localparam int L_W_CONV_OUT     = L_W_IN - L_K + 1;   // 4
  localparam int L_H_POOL_OUT     = L_H_CONV_OUT / 2;   // 3
  ...
  integer                cycle_count;
  integer                ifm_cmd_start_count;
  integer                ifm_dma_wr_count;
  integer                ifm_dma_wr_after_m1_start_count;
  integer                ifm_free_count;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      cycle_count         <= 0;
      ifm_cmd_start_count <= 0;
      ifm_dma_wr_count    <= 0;
      ifm_dma_wr_after_m1_start_count <= 0;
      ifm_free_count      <= 0;
      ...
      // Functional visibility counters for this tiled-IFM test.
      // These hierarchical references are testbench-only checks.
      if (dut.m1_start_s)
  ...
  task automatic init_mem;
    begin
      ...
      $display("TB_INFO: Mode1 initial-IFM tiled layer");
      ...
      $display("TB_INFO: initial tiling target: H_in=%0d > HT=%0d; checker expects multiple IFM DMA commands", L_H_IN, HT);
    end
  endtask
  ...
  // Layer programming
  // --------------------------------------------------------------------------
  task automatic program_mode1_initial_ifm_tiled_layer;
    layer_desc_t cfg;
    begin
      cfg = '0;
      cfg.layer_id      = 0;
  ...
  task automatic check_ofm_against_golden;
    ...
    begin
      ...
      if (ifm_cmd_start_count < 2) begin
        $display("TB_FAIL: initial IFM tiling was not exercised. ifm_cmd_start_count=%0d expected>=2 for H_in=%0d HT=%0d",
                 ifm_cmd_start_count, L_H_IN, HT);
        mismatch = mismatch + 1;
      end

      if (ifm_dma_wr_after_m1_start_count == 0) begin
        $display("TB_WARN: no IFM DMA writes observed after m1_start. ifm_dma_wr_after_m1_start_count=%0d",
                 ifm_dma_wr_after_m1_start_count);
      end
      ...
      if (mismatch == 0) begin
        $display("TB_PASS: exact final OFM matches golden for initial-IFM tiled mode-1 layer");
      end else begin
  ...
  initial begin
    ...
    program_mode1_initial_ifm_tiled_layer();
    repeat (5) @(posedge clk);
    pulse_start();
    ...
    $display("TB_INFO: IFM DMA writes counted = %0d, after m1_start = %0d",
             ifm_dma_wr_count, ifm_dma_wr_after_m1_start_count);
