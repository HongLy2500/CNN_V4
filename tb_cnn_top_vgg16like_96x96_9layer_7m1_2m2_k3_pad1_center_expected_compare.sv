...
module tb_cnn_top_vgg16like_96x96_9layer_7m1_2m2_k3_pad1_center_expected_compare;
  ...
  //
  // Final golden traces spatially through three pools:
  //   final(row,col) -> input(row*8+7, col*8+7)
  // and traces channel backward through all 9 layers using f % C_in.
  // --------------------------------------------------------------------------
  ...
  localparam int OFM_ROW_STRIDE = (W_MAX + PV_MAX - 1) / PV_MAX; // 6 for W=96/PV=16
