...
module tb_cnn_top_2layer_m1_to_m1_large_ofm_tiled;
  ...
  localparam int C_MAX=8, F_MAX=8, W_MAX=8, H_MAX=12, HT=4, K_MAX=3;
  ...
  localparam int CLK_PERIOD_NS=10, MAX_CYCLES=600000;

  // Layer 0: 12x6x3 -> conv3x3 10x4x8 -> pool 5x2x8
  // L0_H_POOL_OUT=5 > HT=4, so the next-layer IFM sourced from OFM buffer is larger than IFM buffer.
  localparam int L0_H_IN=12, L0_W_IN=6, L0_C_IN=3, L0_F_OUT=8, L0_K=3;
  ...
  // Layer 1: 5x2x8 -> conv1x1 5x2x8 -> pool 2x1x8
  ...
  integer ifm_free_count, m1_sm_refill_req_count;
  ...
  always_ff @(posedge clk) begin
    if (!rst_n) begin
      ...
      // Visibility counters for large OFM->IFM tiled refill testing.
      if (dut.m1_start_s)
      ...
      if (m1_sm_refill_req_valid && m1_sm_refill_req_ready)
        m1_sm_refill_req_count <= m1_sm_refill_req_count + 1;
      // New tiled OFM->IFM path bypasses same_mode_refill_manager_m1,
      // so count actual OFM buffer stream commands instead.
  ...
  task automatic init_mem;
    begin
      ...
      $display("TB_INFO: 2-layer Mode1->Mode1 large OFM tiled test");
      ...
      $display("TB_INFO: OFM->IFM tiled target: L1_H_IN=%0d > HT=%0d; checker expects tiled OFM->IFM refill", L1_H_IN, HT);
  ...
  task automatic check_final_ofm;
    ...
    begin
      ...
      if (L0_H_IN > HT && ifm_cmd_start_count < 2) begin
        $display("TB_FAIL: initial DDR->IFM tiling was not exercised. ifm_cmd_start_count=%0d expected>=2 for L0_H_IN=%0d HT=%0d", ifm_cmd_start_count, L0_H_IN, HT);
        fail_count++;
      end
      // For the large-OFM tiled path, the RTL now bypasses same_mode_refill_manager_m1.
      // The old m1_sm_refill_req_count is kept only as debug visibility; the real
      // exercise check is the number of OFM buffer stream commands.
      if (L1_H_IN > HT && ofm_ifm_stream_start_count < EXP_OFM2IFM_STREAMS) begin
        $display("TB_FAIL: OFM->IFM tiled refill may not be fully exercised. stream_start_count=%0d expected>=%0d",
                 ofm_ifm_stream_start_count, EXP_OFM2IFM_STREAMS);
        fail_count++;
      end
      ...
      if ((fail_count==0) && (ofm_mismatch_count==0)) begin
        $display("TB_PASS: exact final OFM matches golden for large OFM tiled mode1->mode1 test");
      end else begin
  ...
  initial begin
    ...
    $display("TB_INFO: old M1 same-mode refill-manager requests accepted = %0d", m1_sm_refill_req_count);
    $display("TB_INFO: OFM->IFM stream starts counted = %0d, stream done counted = %0d",
