...
module tb_cnn_top_9layer_m1_dcp_efficientnet_b0_tablevi_fullscale;
  ...
  // Expected final OFM DDR words generated offline by Python.
  // Keep this path absolute to avoid XSim working-directory ambiguity.
  logic [DDR_WORD_W-1:0] exp_ofm_ddr [0:EXP_OFM_WORDS-1];
  integer exp_init_i;

  initial begin
    for (exp_init_i = 0; exp_init_i < EXP_OFM_WORDS; exp_init_i = exp_init_i + 1) begin
      exp_ofm_ddr[exp_init_i] = 'x;
    end
    $display("TB_INFO: loading expected final OFM from D:/sdh/CNN_V4/expected_final_ofm.hex");
    $readmemh("D:/sdh/CNN_V4/expected_final_ofm.hex", exp_ofm_ddr);
  end
...
final begin
  ...
  task automatic init_mem;
    begin
      ...
      $display("TB_INFO: expected final OFM DDR words=%0d (offline golden compare enabled)", EXP_OFM_WORDS);
  ...
  task automatic dump_ofm_region;
    ...
    integer dump_words;
    begin
      dump_words = (EXP_OFM_WORDS < 64) ? EXP_OFM_WORDS : 64;
      $display("--- OFM DDR region dump, first %0d/%0d expected words ---", dump_words, EXP_OFM_WORDS);
      for (j = 0; j < dump_words; j = j + 1) begin
  ...
  task automatic check_final_ofm;
    ...
    begin
      ...
      // Exact final OFM DDR value check. The expected file is generated offline
      // to avoid huge full-scale golden tensors inside XSim.
      for (i = 0; i < EXP_OFM_WORDS; i = i + 1) begin
        got_word = ddr_mem[`DDR_OFM_BASE + i];
        exp_word = exp_ofm_ddr[i];

        if (got_word !== exp_word) begin
          if (value_mismatch_count < 20) begin
            $display("TB_MISMATCH_FINAL_OFM word=%0d addr=0x%05h got=0x%0h exp=0x%0h",
                     i,
                     (`DDR_OFM_BASE + i),
                     got_word,
                     exp_word);
      ...
      if (value_mismatch_count != 0) begin
        $display("TB_FAIL: final OFM DDR value mismatch count=%0d/%0d", value_mismatch_count, EXP_OFM_WORDS);
        fail_count = fail_count + 1;
      end

      if (fail_count == 0) begin
        $display("TB_PASS: full-scale Mode-1 DCP EfficientNet-B0 prefix completed; count checks and final OFM DDR value check passed. words=%0d", EXP_OFM_WORDS);
      end else begin
        $display("TB_FAIL: total checker failures = %0d", fail_count);
        dump_ofm_region();
        $finish;
      end
    end
