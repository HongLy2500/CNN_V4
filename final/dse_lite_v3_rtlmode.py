#!/usr/bin/env python3
"""
dse_lite_v3.py - Offline DSE-lite for CNN_V4 benchmark layer descriptors.
...
`dse` fields filled. Exported numeric modes follow the CNN_V4 RTL convention:
MODE1 = 0 and MODE2 = 1. The internal optimizer still uses 1/2 labels only for
...
Example
-------
python eval/dse/dse_lite_v3.py \
...
def rtl_mode_id(internal_mode: int) -> int:
    """Map internal DSE mode labels to CNN_V4 RTL encoding.

    Internal search code uses:
      1 = Mode 1, pixel/filter parallelism
      2 = Mode 2, channel/filter parallelism

    RTL package/control convention uses:
      0 = MODE1
      1 = MODE2
    """
    if internal_mode == 1:
    ...
    if internal_mode == 2:
        return 1
    raise ValueError(f"Unsupported internal DSE mode: {internal_mode}")


def mode_name(internal_mode: int) -> str:
    if internal_mode == 1:
        return "MODE1"
    if internal_mode == 2:
        return "MODE2"
...
def write_updated_descriptor_json(model: Dict[str, Any], sol: Solution, path: Path, opts: DseOptions) -> None:
    ...
    for layer, cand in zip(updated["layers"], sol.rows):
        layer["dse"] = {
            "mode": rtl_mode_id(cand.mode),
            "mode_name": mode_name(cand.mode),
...
def print_summary(model: Dict[str, Any], sol: Solution, opts: DseOptions) -> None:
    ...
    print("Mode encoding output   : RTL numeric, 0=MODE1, 1=MODE2")
    print(f"DSE script version     : v3_rtlmode_safe_groups")
