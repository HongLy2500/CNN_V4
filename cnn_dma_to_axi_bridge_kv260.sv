...
module cnn_dma_to_axi_bridge_kv260 #(
  ...
  // Simulation X-hardening for the write issue FSM.
  // In real hardware these flops reset to 0. In simulation, if an X leaks into
  // wr_aw_sent_q / wr_w_sent_q, the original "!flag" style can permanently turn
  // AWVALID/WVALID into X. Treat only a clean 1 as "sent"; X behaves as "not sent".
  logic wr_issue_active_is1;
  logic wr_aw_sent_is1;
  logic wr_w_sent_is1;
  logic wr_aw_hs;
  logic wr_w_hs;
  ...
  always_comb begin
    ...
    // X-hardened decoded status.
    wr_issue_active_is1 = (wr_issue_active_q === 1'b1);
    wr_aw_sent_is1      = (wr_aw_sent_q      === 1'b1);
    wr_w_sent_is1       = (wr_w_sent_q       === 1'b1);
    ...
    // ---------------- Write issue engine ----------------
    // Treat X issue_active as idle. This prevents a simulation-only X from
    // permanently blocking the write channel.
    if (!wr_issue_active_is1 && (wr_count_q != 0)) begin
    ...
    m_axi_awvalid = wr_issue_active_is1 && !wr_aw_sent_is1;
    m_axi_wvalid  = wr_issue_active_is1 && !wr_w_sent_is1;
    m_axi_bready  = 1'b1;

    wr_aw_hs = m_axi_awvalid && m_axi_awready;
    wr_w_hs  = m_axi_wvalid  && m_axi_wready;

    if (wr_aw_hs) wr_aw_sent_n = 1'b1;
    if (wr_w_hs)  wr_w_sent_n  = 1'b1;

    if (wr_issue_active_is1 && ((wr_aw_sent_is1 || wr_aw_hs) && (wr_w_sent_is1 || wr_w_hs))) begin
    ...
    end else if (!wr_issue_active_is1) begin
      wr_aw_sent_n = 1'b0;
      wr_w_sent_n  = 1'b0;
    end

    if (m_axi_bvalid) begin
      if (m_axi_bresp != 2'b00) error_n = 1'b1;
      if (wr_outstanding_q != 0) wr_outstanding_n = wr_outstanding_q - 1'b1;
    end
  end
  ...
  assign busy             = rd_cmd_active_q || rd_wait_data_q ||
                            (wr_count_q != 0) || wr_issue_active_q ||
                            (wr_outstanding_q != 0);
