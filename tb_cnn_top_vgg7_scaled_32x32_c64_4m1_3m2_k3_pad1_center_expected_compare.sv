...
module tb_cnn_top_vgg7_scaled_32x32_c64_4m1_3m2_k3_pad1_center_expected_compare;
  ...
  //
  // For this benchmark:
  //   L0 no-pool  W=32, pack=PV=16   -> ceil(32/16)=2
  //   L1 pool     W=16, pack=PV/2=8  -> ceil(16/8)=2
  //   L2 no-pool  W=16, pack=PV=16   -> ceil(16/16)=1
  //   L3 pool     W=8,  pack=PV/2=8  -> ceil(8/8)=1
  //   L4-L6 M2    W=8/4, pack=PC=16  -> 1
  // So OFM_ROW_STRIDE=2 is sufficient; do not allocate H_MAX*W_MAX words.
  // --------------------------------------------------------------------------
  localparam int OFM_ROW_STRIDE = 2;
  localparam int OFM_BANK_DEPTH = H_MAX * OFM_ROW_STRIDE;   // 32*2 = 64 words/bank
  localparam int OFM_LINEAR_DEPTH = F_MAX * OFM_BANK_DEPTH; // 128*64 = 8192 words total logical span
  ...
  task automatic check_final_ofm;
    ...
    begin
      ...
      if (mismatch != 0) begin
        dump_ofm_region();
        $fatal(1, "TB_FAIL: VGG7-like 32x32 7-layer 4M1+3M2 K3/P1 final OFM mismatch count=%0d", mismatch);
      end
    end
  endtask
  ...
  task automatic print_banner;
    begin
      $display("TB_INFO: VGG7-like 32x32 7 conv layers, 4M1+3M2, K=3 stride=1 pad=1");
  ...
  initial begin
    ...
    $display("TB_INFO: VGG7-like 32x32 7-layer test done after %0d cycles", cycle_count);
    ...
    check_final_ofm();
    $display("TB_PASS: VGG7-like 32x32 7-layer 4M1+3M2 K3/P1 center-tap expected-compare passed");
    $finish;
  end
endmodule
