...
module tb_cnn_top_9layer_m2_dcp_efficientnet_b0_tablevi_fullsize_k1_expected_compare;
  ...
  task automatic print_banner;
    begin
      ...
      $display("TB_INFO: full-size W224 Mode2 regression; IFM DDR layout uses corrected WT=PC contract");
  ...
  initial begin
    ...
    check_final_ofm();
    $display("TB_PASS: 9-layer Mode2 DCP/EfficientNet-B0 Table-VI-style full-size K1 expected-compare passed. final_value=%0d elements=%0d ddr_words=%0d", EXPECTED_FINAL_VALUE, EXPECTED_FINAL_ELEMENTS, EXPECTED_OFM_DDR_WORDS);
    $finish;
  end
endmodule
