...
module tb_cnn_top_9layer_m1_efficientnet_like_prefix_k3only;
  ...
  localparam int L5_M1_PHYS_WORDS=(L5_M1_LOGICAL_BUNDLES+PV_M1-1)/PV_M1;
  ...
  localparam int L6_M1_PHYS_WORDS=(L6_M1_LOGICAL_BUNDLES+PV_M1-1)/PV_M1;
  localparam int L6_M1_WGT_DDR_WORDS=L6_M1_PHYS_WORDS*WGT_SUBWORDS;
  localparam int L7_NUM_FGROUP=(L7_F_OUT+PF_M1-1)/PF_M1;
  localparam int L7_M1_LOGICAL_BUNDLES=L7_NUM_FGROUP*L7_C_IN*L7_K*L7_K;
  localparam int L7_M1_PHYS_WORDS=(L7_M1_LOGICAL_BUNDLES+PV_M1-1)/PV_M1;
  localparam int L7_M1_WGT_DDR_WORDS=L7_M1_PHYS_WORDS*WGT_SUBWORDS;
  localparam int L8_NUM_FGROUP=(L8_F_OUT+PF_M1-1)/PF_M1;
  localparam int L8_M1_LOGICAL_BUNDLES=L8_NUM_FGROUP*L8_C_IN*L8_K*L8_K;
  localparam int L8_M1_PHYS_WORDS=(L8_M1_LOGICAL_BUNDLES+PV_M1-1)/PV_M1;
  ...
  localparam int L5_OFM2IFM_COL_BLKS=(L5_W_IN+PV_M1-1)/PV_M1;
  localparam int L5_OFM2IFM_CH_BLKS=(L5_C_IN+PF_M1-1)/PF_M1;
  localparam int L6_OFM2IFM_COL_BLKS=(L6_W_IN+PV_M1-1)/PV_M1;
  localparam int L6_OFM2IFM_CH_BLKS=(L6_C_IN+PF_M1-1)/PF_M1;
  localparam int L7_OFM2IFM_COL_BLKS=(L7_W_IN+PV_M1-1)/PV_M1;
  localparam int L7_OFM2IFM_CH_BLKS=(L7_C_IN+PF_M1-1)/PF_M1;
  localparam int L8_OFM2IFM_COL_BLKS=(L8_W_IN+PV_M1-1)/PV_M1;
  localparam int L8_OFM2IFM_CH_BLKS=(L8_C_IN+PF_M1-1)/PF_M1;
...
// ============================================================
// DEBUG: EfficientNet-like K=3 only
// Focus:
//   1) L1 pool_en=0 OFM write completeness
//   2) L1->L2 OFM->IFM refill completeness
//   3) L2 compute/write progress
// ============================================================

localparam int DBG_L1_EXP_WRITE_PULSES = 4050;
localparam int DBG_L1_EXP_VALID_ELEMS  = 16200;

localparam int DBG_L12_EXP_STREAM_CMDS = 2070;

localparam int DBG_L2_EXP_WRITE_PULSES = 1323;
localparam int DBG_L2_EXP_VALID_ELEMS  = 4410;

integer dbg_l1_ofm_write_pulse_count;
integer dbg_l1_ofm_valid_elem_count;
logic   dbg_l1_m1_done_seen_q;
logic   dbg_l1_write_count_done_q;
logic   dbg_l1_handoff_armed_q;

integer dbg_l12_stream_start_count;
integer dbg_l12_stream_done_count;

integer dbg_l2_cycle_count;
integer dbg_l2_ofm_write_pulse_count;
integer dbg_l2_ofm_valid_elem_count;
integer dbg_l2_free_count;
logic   dbg_seen_layer2_start_q;
logic   dbg_l2_m1_done_seen_q;
logic   dbg_l2_ofm_layer_done_seen_q;

always_ff @(posedge clk or negedge rst_n) begin
  if (!rst_n) begin
    dbg_l1_ofm_write_pulse_count <= 0;
    dbg_l1_ofm_valid_elem_count  <= 0;
    dbg_l1_m1_done_seen_q        <= 1'b0;
    dbg_l1_write_count_done_q    <= 1'b0;
    dbg_l1_handoff_armed_q       <= 1'b0;

    dbg_l12_stream_start_count   <= 0;
    dbg_l12_stream_done_count    <= 0;

    dbg_l2_cycle_count           <= 0;
    dbg_l2_ofm_write_pulse_count <= 0;
    dbg_l2_ofm_valid_elem_count  <= 0;
    dbg_l2_free_count            <= 0;
    dbg_seen_layer2_start_q      <= 1'b0;
    dbg_l2_m1_done_seen_q        <= 1'b0;
    dbg_l2_ofm_layer_done_seen_q <= 1'b0;
  end else begin

    // --------------------------------------------------------
    // 1) Monitor L1 OFM writes
    // L1: 47x47x8 -> conv 45x45x8, pool_en=0
    // Expected write pulses = 45*45*ceil(8/4)=4050
    // Expected valid elems  = 45*45*8=16200
    // --------------------------------------------------------
    if (dut.dbg_layer_idx == 1 && dut.m1_ofm_write_en_s) begin
      dbg_l1_ofm_write_pulse_count <= dbg_l1_ofm_write_pulse_count + 1;
      dbg_l1_ofm_valid_elem_count  <= dbg_l1_ofm_valid_elem_count + dut.m1_ofm_write_count_s;

      if (dbg_l1_ofm_write_pulse_count < 20 ||
          (dbg_l1_ofm_write_pulse_count % 500) == 0) begin
        $display("DBG_L1_OFM_WRITE t=%0t cnt=%0d row=%0d col_base=%0d fbase=%0d count=%0d data0=%0h data1=%0h data2=%0h data3=%0h",
                 $time,
                 dbg_l1_ofm_write_pulse_count + 1,
      ...
      if (!dbg_l1_write_count_done_q &&
          ((dbg_l1_ofm_valid_elem_count + dut.m1_ofm_write_count_s) >= DBG_L1_EXP_VALID_ELEMS)) begin
        dbg_l1_write_count_done_q <= 1'b1;
        dbg_l1_handoff_armed_q    <= 1'b1;

        $display("DBG_L1_WRITE_COUNT_DONE t=%0t pulses=%0d/%0d valid_elem=%0d/%0d",
                 $time,
                 dbg_l1_ofm_write_pulse_count + 1,
                 DBG_L1_EXP_WRITE_PULSES,
                 dbg_l1_ofm_valid_elem_count + dut.m1_ofm_write_count_s,
                 DBG_L1_EXP_VALID_ELEMS);
      end
    end

    if (dut.dbg_layer_idx == 1 && dut.m1_done_s && !dbg_l1_m1_done_seen_q) begin
      dbg_l1_m1_done_seen_q  <= 1'b1;
      dbg_l1_handoff_armed_q <= 1'b1;

      $display("DBG_L1_M1_DONE t=%0t l1_write_pulse=%0d/%0d l1_valid_elem=%0d/%0d",
               $time,
               dbg_l1_ofm_write_pulse_count,
               DBG_L1_EXP_WRITE_PULSES,
               dbg_l1_ofm_valid_elem_count,
               DBG_L1_EXP_VALID_ELEMS);
    end

    // --------------------------------------------------------
    // 2) Detect entering L2
    // --------------------------------------------------------
    if (!dbg_seen_layer2_start_q && dut.dbg_layer_idx == 2) begin
      dbg_seen_layer2_start_q <= 1'b1;

      $display("DBG_L2_START t=%0t l1_write_pulse=%0d/%0d l1_valid_elem=%0d/%0d l12_stream_start=%0d l12_stream_done=%0d",
               $time,
               dbg_l1_ofm_write_pulse_count,
               DBG_L1_EXP_WRITE_PULSES,
               dbg_l1_ofm_valid_elem_count,
               DBG_L1_EXP_VALID_ELEMS,
               dbg_l12_stream_start_count,
               dbg_l12_stream_done_count);
    end

    // --------------------------------------------------------
    // 3) Count L1->L2 OFM->IFM stream commands
    //
    // Count two phases:
    //   A. initial tile while still at dbg_layer_idx == 1,
    //      after L1 compute/write is done
    //   B. runtime refill while dbg_layer_idx == 2,
    //      before L2 m1_done
    // --------------------------------------------------------
    if (dut.ofm_ifm_stream_start_s &&
        (
          (dut.dbg_layer_idx == 1 && dbg_l1_handoff_armed_q && !dbg_seen_layer2_start_q) ||
          (dut.dbg_layer_idx == 2 && dbg_seen_layer2_start_q && !dbg_l2_m1_done_seen_q)
        )) begin

      dbg_l12_stream_start_count <= dbg_l12_stream_start_count + 1;

      if (dbg_l12_stream_start_count < 50 ||
          (dbg_l12_stream_start_count % 100) == 0) begin
        $display("DBG_L12_STREAM_START t=%0t cnt=%0d layer=%0d row=%0d col_base=%0d slot=%0d ch_blk=%0d",
                 $time,
                 dbg_l12_stream_start_count + 1,
                 dut.dbg_layer_idx,
    ...
    if (dut.ofm_ifm_stream_done_s &&
      ...
      dbg_l12_stream_done_count <= dbg_l12_stream_done_count + 1;

      if (dbg_l12_stream_done_count < 50 ||
          (dbg_l12_stream_done_count % 100) == 0) begin
        $display("DBG_L12_STREAM_DONE t=%0t cnt=%0d layer=%0d",
                 $time,
                 dbg_l12_stream_done_count + 1,
                 dut.dbg_layer_idx);
      end
    end
    ...
    // L2: 45x45x8 -> conv 43x43x10 -> pool 21x21x10
    // Expected write pulses = 21*21*ceil(10/4)=1323
    // Expected valid elems  = 21*21*10=4410
    // --------------------------------------------------------
    if (dut.dbg_layer_idx == 2) begin
      dbg_l2_cycle_count <= dbg_l2_cycle_count + 1;

      if (dut.ifm_m1_free_valid) begin
        dbg_l2_free_count <= dbg_l2_free_count + 1;

        if (dbg_l2_free_count < 20 ||
            (dbg_l2_free_count % 20) == 0) begin
          $display("DBG_L2_FREE t=%0t cnt=%0d row_base_l=%0d free_slot=%0d free_row_g=%0d m1_out_row=%0d",
                   $time,
                   dbg_l2_free_count + 1,
      ...
      if (dut.m1_ofm_write_en_s) begin
        dbg_l2_ofm_write_pulse_count <= dbg_l2_ofm_write_pulse_count + 1;
        dbg_l2_ofm_valid_elem_count  <= dbg_l2_ofm_valid_elem_count + dut.m1_ofm_write_count_s;

        if (dbg_l2_ofm_write_pulse_count < 40 ||
            (dbg_l2_ofm_write_pulse_count % 100) == 0) begin
          $display("DBG_L2_OFM_WRITE t=%0t cnt=%0d row=%0d col_base=%0d fbase=%0d count=%0d data0=%0h data1=%0h data2=%0h data3=%0h",
                   $time,
                   dbg_l2_ofm_write_pulse_count + 1,
                   dut.m1_ofm_write_row_s,
                   dut.m1_ofm_write_col_base_s,
                   dut.m1_ofm_write_filter_base_s,
                   dut.m1_ofm_write_count_s,
      ...
      if (dut.m1_done_s && !dbg_l2_m1_done_seen_q) begin
        dbg_l2_m1_done_seen_q <= 1'b1;

        $display("DBG_L2_M1_DONE t=%0t l12_stream_start=%0d/%0d l12_stream_done=%0d/%0d l2_write_pulse=%0d/%0d l2_valid_elem=%0d/%0d",
                 $time,
                 dbg_l12_stream_start_count,
                 DBG_L12_EXP_STREAM_CMDS,
                 dbg_l12_stream_done_count,
                 DBG_L12_EXP_STREAM_CMDS,
                 dbg_l2_ofm_write_pulse_count,
                 DBG_L2_EXP_WRITE_PULSES,
                 dbg_l2_ofm_valid_elem_count,
                 DBG_L2_EXP_VALID_ELEMS);
      end

      if (dut.ofm_layer_write_done_s && !dbg_l2_ofm_layer_done_seen_q) begin
        dbg_l2_ofm_layer_done_seen_q <= 1'b1;

        $display("DBG_L2_OFM_LAYER_WRITE_DONE t=%0t l2_write_pulse=%0d/%0d l2_valid_elem=%0d/%0d",
                 $time,
                 dbg_l2_ofm_write_pulse_count,
                 DBG_L2_EXP_WRITE_PULSES,
                 dbg_l2_ofm_valid_elem_count,
      ...
      if ((dbg_l2_cycle_count % 100000) == 0) begin
        $display("DBG_L2_STATUS t=%0t cycle=%0d l12_stream_start=%0d/%0d l12_stream_done=%0d/%0d l2_write_pulse=%0d/%0d l2_valid_elem=%0d/%0d free=%0d m1_busy=%0b m1_done=%0b ofm_done=%0b m1_step=%0b out_row=%0d out_col=%0d f_group=%0d c_iter=%0d ky=%0d kx=%0d",
                 $time,
                 dbg_l2_cycle_count,
                 dbg_l12_stream_start_count,
                 DBG_L12_EXP_STREAM_CMDS,
...
final begin
  $display("DBG_L1_SUMMARY ofm_write_pulse=%0d expected=%0d",
           dbg_l1_ofm_write_pulse_count,
           DBG_L1_EXP_WRITE_PULSES);

  $display("DBG_L1_SUMMARY ofm_valid_elem=%0d expected=%0d",
           dbg_l1_ofm_valid_elem_count,
           DBG_L1_EXP_VALID_ELEMS);

  $display("DBG_L1_SUMMARY m1_done_seen=%0b write_count_done=%0b handoff_armed=%0b",
           dbg_l1_m1_done_seen_q,
           dbg_l1_write_count_done_q,
           dbg_l1_handoff_armed_q);

  $display("DBG_L12_SUMMARY stream_start=%0d expected=%0d",
           dbg_l12_stream_start_count,
           DBG_L12_EXP_STREAM_CMDS);

  $display("DBG_L12_SUMMARY stream_done=%0d expected=%0d",
           dbg_l12_stream_done_count,
           DBG_L12_EXP_STREAM_CMDS);

  $display("DBG_L2_SUMMARY ofm_write_pulse=%0d expected=%0d",
           dbg_l2_ofm_write_pulse_count,
           DBG_L2_EXP_WRITE_PULSES);

  $display("DBG_L2_SUMMARY ofm_valid_elem=%0d expected=%0d",
           dbg_l2_ofm_valid_elem_count,
           DBG_L2_EXP_VALID_ELEMS);

  $display("DBG_L2_SUMMARY free_count=%0d m1_done_seen=%0b ofm_layer_done_seen=%0b",
           dbg_l2_free_count,
           dbg_l2_m1_done_seen_q,
           dbg_l2_ofm_layer_done_seen_q);
end
  ...
  task automatic init_mem;
    begin
      ...
      $display("TB_INFO: 9-layer EfficientNet-like Mode1 prefix test");
      ...
      $display("TB_INFO: benchmark intent: EfficientNet-like mode-1 prefix; all K=3; no depthwise/SE/skip ops");
  ...
  task automatic program_layers;
    ...
    begin
      ...
      cfg5.pv_m1=PV_M1; cfg5.pf_m1=PF_M1; cfg5.pc_m2=PC_MODE2; cfg5.pf_m2=PF_MODE2;
      ...
      cfg6.pv_m1=PV_M1; cfg6.pf_m1=PF_M1; cfg6.pc_m2=PC_MODE2; cfg6.pf_m2=PF_MODE2;
      ...
      cfg7.pv_m1=PV_M1; cfg7.pf_m1=PF_M1; cfg7.pc_m2=PC_MODE2; cfg7.pf_m2=PF_MODE2;
      ...
      cfg8.pv_m1=PV_M1; cfg8.pf_m1=PF_M1; cfg8.pc_m2=PC_MODE2; cfg8.pf_m2=PF_MODE2;
