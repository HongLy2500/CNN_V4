...
`ifndef SYNTHESIS
longint unsigned dbg_m1_ext_step_cnt;
longint unsigned dbg_m1_ctrl_step_cnt;
longint unsigned dbg_m1_mac_req_cnt;
longint unsigned dbg_m1_stall_ext_hold_cnt;
longint unsigned dbg_m1_stall_data_cnt;
longint unsigned dbg_m1_stall_weight_cnt;
longint unsigned dbg_m1_weight_req_cnt;
longint unsigned dbg_m1_weight_load_cnt;

always_ff @(posedge clk or negedge rst_n) begin
  if (!rst_n || start) begin
    dbg_m1_ext_step_cnt       <= 0;
    dbg_m1_ctrl_step_cnt      <= 0;
    dbg_m1_mac_req_cnt        <= 0;
    dbg_m1_stall_ext_hold_cnt <= 0;
    dbg_m1_stall_data_cnt     <= 0;
    dbg_m1_stall_weight_cnt   <= 0;
    dbg_m1_weight_req_cnt     <= 0;
    dbg_m1_weight_load_cnt    <= 0;
  end else begin
    if (ctrl_mac_en) begin
      dbg_m1_mac_req_cnt <= dbg_m1_mac_req_cnt + 1;

      if (!step_en)
        dbg_m1_stall_ext_hold_cnt <= dbg_m1_stall_ext_hold_cnt + 1;
      else if (!data_ready_q)
        dbg_m1_stall_data_cnt <= dbg_m1_stall_data_cnt + 1;
      else if (!weight_valid_q)
        dbg_m1_stall_weight_cnt <= dbg_m1_stall_weight_cnt + 1;
    end

    if (step_en)
      dbg_m1_ext_step_cnt <= dbg_m1_ext_step_cnt + 1;

    if (ctrl_step_en)
      dbg_m1_ctrl_step_cnt <= dbg_m1_ctrl_step_cnt + 1;

    if (wb_rd_en_i)
      dbg_m1_weight_req_cnt <= dbg_m1_weight_req_cnt + 1;

    if (weight_load_en)
      dbg_m1_weight_load_cnt <= dbg_m1_weight_load_cnt + 1;

    if (ctrl_done) begin
      $display("PERF_M1_CE t=%0t ext_step=%0d ctrl_step=%0d mac_req=%0d stall_ext_hold=%0d stall_data=%0d stall_weight=%0d w_req=%0d w_load=%0d",
        $time,
        dbg_m1_ext_step_cnt,
        dbg_m1_ctrl_step_cnt,
        dbg_m1_mac_req_cnt,
        dbg_m1_stall_ext_hold_cnt,
        dbg_m1_stall_data_cnt,
        dbg_m1_stall_weight_cnt,
        dbg_m1_weight_req_cnt,
        dbg_m1_weight_load_cnt
      );
    end
  end
end
