...
module tb_cnn_top_vgg7_scaled_32x32_c64_p128_pc8_4m1_3m2_k3_pad1_center_expected_compare;
  import cnn_layer_desc_pkg::*;

  // --------------------------------------------------------------------------
  // VGG7-like 32x32 7-layer regression
  // Channel scale small:
  //   16,16,32,32,64,64,64
  // L0-L3 : Mode1
  // L4-L6 : Mode2
  // PV=8, PC=8, PF=16, PTOTAL=128. Transition after L3 has W=8 == PC.
  ...
  localparam int PTOTAL = PC * PF; // 8*16 = 128
  localparam int PV_M1 = 8;
  ...
  // --------------------------------------------------------------------------
  // Compact OFM storage for this 32x32 7-layer benchmark.
  // One OFM memory word stores PV_MAX (=8) scalar pixels/lanes. Therefore the
  // physical row stride is the maximum number of packed words needed by any
  // layer row, not W_MAX.
  //
  // For this benchmark:
  //   L0 no-pool  W=32, pack=PV=8    -> ceil(32/8)=4
  //   L1 pool     W=16, pack=PV/2=4  -> ceil(16/4)=4
  //   L2 no-pool  W=16, pack=PV=8    -> ceil(16/8)=2
  //   L3 pool     W=8,  pack=PV/2=4  -> ceil(8/4)=2
  //   L4-L6 M2    W=8/4, pack=PC=8   -> 1
  // So OFM_ROW_STRIDE=4 is sufficient; do not allocate H_MAX*W_MAX words.
  // --------------------------------------------------------------------------
  localparam int OFM_ROW_STRIDE = 4;
  localparam int OFM_BANK_DEPTH = H_MAX * OFM_ROW_STRIDE;   // 32*4 = 128 words/bank
  localparam int OFM_LINEAR_DEPTH = F_MAX * OFM_BANK_DEPTH; // 64*128 = 8192 words total logical span
  ...
  localparam int NUM_LAYERS = 7;
  localparam int LK = 3;
  localparam int PAD = 1;

  // --------------------------------------------------------------------------
  // 7-layer VGG-like 32x32 shape table.
  ...
  function automatic logic [DATA_W-1:0] expected_final_byte(input int row, input int col, input int out_ch);
    begin
      expected_final_byte = sat8(ref_layer_value(6, row, col, out_ch));
    end
  endfunction
    ...
    begin
      ...
      for (int j = 0; j < EXPECTED_OFM_DDR_WORDS; j++) begin
        ...
        if (got_word !== exp_word) begin
          ...
          dec_row = dec_rem / L6_STORED_GROUPS;
          dec_grp = dec_rem % L6_STORED_GROUPS;
      ...
      if (mismatch != 0) begin
        dump_ofm_region();
        $fatal(1, "TB_FAIL: VGG7-like 32x32 C64 P128 PC8 7-layer 4M1+3M2 K3/P1 final OFM mismatch count=%0d", mismatch);
      end
    end
  endtask
  ...
  task automatic print_banner;
    begin
      $display("TB_INFO: VGG7-like 32x32 C64 P128 PC8 7 conv layers, 4M1+3M2, K=3 stride=1 pad=1");
  ...
  initial begin
    ...
    $display("TB_INFO: VGG7-like 32x32 C64 P128 PC8 7-layer test done after %0d cycles", cycle_count);
    ...
    check_final_ofm();
    $display("TB_PASS: VGG7-like 32x32 C64 P128 PC8 7-layer 4M1+3M2 K3/P1 center-tap expected-compare passed");
    $finish;
  end
endmodule
