...
module tb_cnn_top_9layer_m2_dcp_efficientnet_b0_tablevi_fullsize_k1_expected_compare;
  import cnn_layer_desc_pkg::*;

  // --------------------------------------------------------------------------
  // 9-layer Mode-2 DCP/EfficientNet-B0 Table-VI-style full-size test.
  //
  // Purpose:
  //   This is the full spatial-size companion to the W32 Mode-2 regression.
  //   It uses the same 9-layer EfficientNet-B0-style channel trend and the
  //   same Mode-2 DCP-CNN point Pc=32, Pf=64, but restores the first-layer
  //   input size to 224x224x3 like the full-size Mode-1 EfficientNet test.
  //
  ...
  //   for W=224 without mixing in K>1 cross-tile/halo behavior. A later K=3
  ...
  //
  // This stresses:
  //   - Mode-2 first-layer DDR->IFM preload over 7 horizontal tiles
  //   - W=224 with WT=PC=32, i.e. ceil(224/32)=7 col tiles
  ...
  // Do not use W_MAX as the physical row stride here: W_MAX=224 would make
  // DEPTH=224*224 per bank and can crash XSim with huge unpacked arrays.
  // The required physical groups per row are ceil(W_MAX/PC)=7.
  ...
  // K is set to 1 here so the full-size W224 regression can validate
  // multi-tile Mode-2 storage without mixing in K>1 halo behavior yet.
  localparam int L0_H_IN=224, L0_W_IN=224, L0_C_IN=3, L0_F_OUT=32, L0_K=1, L0_POOL_EN=1;
  ...
  always_ff @(posedge clk) begin
    if (rst_n) begin
      if (dut.ifm_ofm_wr_en_s && dut.ifm_ofm_wr_ready_s && (dbg_layer_idx == 0)) begin
        int global_col;
        logic [DATA_W-1:0] exp_l0;
        global_col = int'(dut.ofm_ifm_stream_col_base_s) + int'(dut.ifm_ofm_wr_bank_s);
        exp_l0 = expected_l0_stream_value(global_col);

        l0_stream_checked_count <= l0_stream_checked_count + 1;

        if ((global_col >= L1_W_IN) ||
            (dut.ifm_ofm_wr_col_idx_s != 0) ||
            (dut.ifm_ofm_wr_data_s[0*DATA_W +: DATA_W] !== exp_l0) ||
            (dut.ifm_ofm_wr_data_s[1*DATA_W +: DATA_W] !== exp_l0) ||
            (dut.ifm_ofm_wr_data_s[31*DATA_W +: DATA_W] !== exp_l0)) begin
          l0_stream_mismatch_count <= l0_stream_mismatch_count + 1;
          if (l0_stream_mismatch_count < 16) begin
            $display("TB_MISMATCH_L0_M2_STREAM t=%0t cycle=%0d row=%0d col_base=%0d bank_col_l=%0d global_col=%0d cgrp=%0d got0=%0d got1=%0d got31=%0d exp=%0d keep=%h",
              $time,
              cycle_count,
  ...
  function automatic logic [DATA_W-1:0] expected_l0_stream_value;
    ...
    begin
      ...
      // columns belong to the same tile for this W224/PC32 regression.
  ...
  task automatic init_mem;
    ...
    begin
      ...
      //   [tile_x][row][cgrp][col_l], lanes = PC channels.
      // This full-size W224 regression has 7 tile_x blocks when PC=32.
      ...
      // Expected first-layer IFM reads = 7 * 224 * ceil(3/32) * 32 = 50176.
      word_idx = 0;
  ...
  task automatic check_final_ofm;
    ...
    begin
      ...
      if (mismatch != 0) begin
        dump_ofm_region();
        $fatal(1, "TB_FAIL: 9-layer Mode2 full-size K1 final OFM mismatch count=%0d", mismatch);
      end
    end
  endtask
  ...
  task automatic print_banner;
    begin
      $display("TB_INFO: 9-layer Mode2 DCP/EfficientNet-B0 Table-VI-style full-size K1 expected-compare test");
      $display("TB_INFO: full-size W224 Mode2 multi-tile regression; IFM tile pattern checks true tile scheduling");
      ...
      $display("TB_INFO: expected L0->L1 tile-pattern stream words=%0d", L1_H_IN*L1_COLBLKS*L1_NUM_CGROUP*PC);
  ...
  initial begin
    ...
    if (l0_stream_checked_count != (L1_H_IN*L1_COLBLKS*L1_NUM_CGROUP*PC)) $fatal(1, "TB_FAIL: unexpected L0->L1 stream data word count");
    ...
    check_final_ofm();
    $display("TB_PASS: 9-layer Mode2 full-size K1 multi-tile pattern expected-compare passed. final_value=%0d elements=%0d ddr_words=%0d", EXPECTED_FINAL_VALUE, EXPECTED_FINAL_ELEMENTS, EXPECTED_OFM_DDR_WORDS);
    $finish;
  end
endmodule
