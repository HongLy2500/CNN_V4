module same_mode_refill_manager_m1 #(
  parameter int HT         = 8,
  parameter int H_W        = 16,
  parameter int COLG_W     = 16,
  parameter int CHG_W      = 16,
  ...
  input  logic [$clog2(HT)-1:0]   free_row_slot_l,
  input  logic [H_W-1:0]          free_row_g,
  input  logic [COLG_W-1:0]       free_col_blk_g,
  input  logic [CHG_W-1:0]        free_ch_blk_g,
  ...
  input  logic [COLG_W-1:0]       ready_col_blk_g,
  input  logic [CHG_W-1:0]        ready_ch_blk_g,
  ...
  output logic [$clog2(HT)-1:0]   refill_row_slot_l,
  output logic [H_W-1:0]          refill_row_g,
  output logic [COLG_W-1:0]       refill_col_blk_g,
  output logic [CHG_W-1:0]        refill_ch_blk_g,
...
`ifndef SYNTHESIS
  always_ff @(posedge clk) begin
    if (rst_n && free_valid) begin
      if ((free_row_g >= cfg_next_h_in) || (free_col_blk_g * cfg_next_pv >= cfg_next_w_in) || (free_ch_blk_g * cfg_next_pf >= cfg_next_c_in)) begin
        $error("same_mode_refill_manager_m1: free token out of next-layer range.");
      end
    end
    if (rst_n && ready_valid) begin
      if ((ready_row_g >= cfg_cur_h_out) || (ready_col_blk_g * cfg_next_pv >= cfg_cur_w_out) || (ready_ch_blk_g * cfg_next_pf >= cfg_cur_f_out)) begin
        $error("same_mode_refill_manager_m1: ready token out of current-layer range.");
      end
    end
  end
`endif
